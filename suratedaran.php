      ...
      if ($sql_dbs) {
          while ($row = mysqli_fetch_array($sql_dbs)) {
              ...
              if (preg_match('/^sas_(\d+)$/', $dbname, $matches)) {
                  $thn = $matches[1];
                  // Gunakan $tahunsklh jika tersedia (dari secure.php), atau fallback ke session
...
<?php } ?>
                          ...
                          <div class="card-body">
                              <div class="table-responsive" id="data-edaran">
      ...
      <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div class="modal-footer">
                  ...
                  <button type="button" class="btn btn-danger" id="tombolKonfirmasiHapus">Delete</button>
              </div>
          </div>
      </div>
  </div>
...
<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
  ...
  <script>
              ...
              $.ajax({
                  ...
                  success: function(response) {
                      if (response.status === 'success') {
                          $('#data-edaran').html(response.data.table);
                          ...
                          $('#data-edaran').html('<div class="alert alert-danger">Gagal memuat data.</div>');
                  ...
                  error: function(jqXHR, textStatus, errorThrown) {
                      ...
                      $('#data-edaran').html('<div class="alert alert-danger">Error: ' + errorThrown + '</div>');
                  },
                  complete: function() {
    ...
    makeModalsDraggable();
          ...
          $('#data-edaran').on('click', '.tombol-view', function() {
              var id = $(this).data('id');
              var tahun = $('#year-filter').val();
                      ...
                      if (response.status === 'success') {
                          ...
                          if (file) {
                              ...
                              if (files.length === 1 && files[0].trim() !== '') {
                                  var path = encodeURI('file/berkas-edaran/' + files[0]);
                                  var iframe = $('<iframe>', {
                                      src: path,
                                  ...
                                  files.forEach(function(f, index) {
                                      if(f.trim() !== '') {
                                          var path = encodeURI('file/berkas-edaran/' + f);
          ...
          $('#data-edaran').on('click', '.tombol-edit', function() {
              var id = $(this).data('id');
                  ...
                  success: function(data) {
                      if (data.status === 'success') {
                          ...
                          if (data.data.pdf) {
                              ...
                              files.forEach(function(f, index) {
                                  if(f.trim() !== '') {
                                      var pdfPath = encodeURI('file/berkas-edaran/' + f);
          ...
          $('#data-edaran').on('click', '.tombol-hapus', function() {
              var id = $(this).data('id');
              ...
              $.ajax({
                  ...
                  success: function(data) {
                      if (data.status === 'success') {
                          ...
                          if (data.data.pdf) {
                              var fullUrl = encodeURI('file/berkas-edaran/' + data.data.pdf);
                              // Set iframe
